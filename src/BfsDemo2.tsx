...
//
// === 新增：兄弟节点错位函数（避免共线遮挡，边仍是直线） ===
function staggerSiblings(cy: any, gap = 60, tol = 8) {
  if (!cy) return;

  cy.nodes().forEach((p: any) => {
    // p 的所有子节点（出边的目标）
    const kids = p.outgoers("edge").targets();
    const m = kids.length;
    if (m <= 1) return;

    // 若这些子节点 y 值几乎相等（在同一条水平线上），才需要错位
    const ys = kids.map((k: any) => k.position("y"));
    const aligned = Math.max(...ys) - Math.min(...ys) <= tol;
    if (!aligned) return;

    // 按 x 从左到右排序，围绕中点对称在 y 方向错位：..., -gap, 0, +gap, ...
    const arr = kids.toArray().sort(
      (a: any, b: any) => a.position("x") - b.position("x")
    );
    const mid = (m - 1) / 2;
    arr.forEach((k: any, i: number) => {
      const pos = k.position();
      const offset = (i - mid) * gap;
      k.position({ x: pos.x, y: pos.y + offset });
    });
  });
}
...
const BfsDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (cyRef.current) {
      ...
      layout.run();
      // 布局结束后会触发 layoutstop，我们再做兄弟错位，见 cy 回调中的绑定
    }
  }, [startNode]);
  ...
  const stylesheet = [
    ...
    },
    {
      ...
      style: {
        width: 2,
        // 如果没给 data(label)，这行不会显示文字，保留也无妨
        label: "data(label)",
        "font-size": 10,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr 260px", gap: 12, padding: 12 }}>
        ...
        <div style={{ display: "grid", gap: 10, alignContent: "start" }}>
          <div style={{ fontSize: 13, color: "#334155" }}>
            <div>
              <b>Start node:</b>{" "}
              <span style={{ color: "#2563eb" }}>{startNode}</span>
            </div>
            <div style={{ marginTop: 4 }}>
              👉 Click any node on the graph to set it as the start
              {isPlaying && (
                <span style={{ color: "#ef4444" }}> (pause first)</span>
              )}
            </div>
          </div>
          ...
          <div style={{ marginTop: 16, fontSize: 12, color: "#64748b" }}>
            ...
            <div style={{ display: "grid", gap: 6, marginTop: 6 }}>
              <div>
                <span style={{ display: "inline-block", width: 12, height: 12, background: "#2563eb", marginRight: 8 }} />
                Current
              </div>
              <div>
                <span style={{ display: "inline-block", width: 12, height: 12, background: "#eab308", marginRight: 8 }} />
                In Queue
              </div>
              <div>
                <span style={{ display: "inline-block", width: 12, height: 12, background: "#22c55e", marginRight: 8 }} />
                Visited
              </div>
              <div>
                <span style={{ display: "inline-block", width: 12, height: 2, background: "#ef4444", marginRight: 8, verticalAlign: "middle" }} />
                Enqueued edges
              </div>
            </div>
          </div>
        ...
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden" }}>
          <CytoscapeComponent
            ...
            cy={(cy) => {
              ...
              // 布局完成后：做一次兄弟错位，避免共线遮挡
              cy.off("layoutstop"); // 避免重复绑定
              cy.on("layoutstop", () => {
                staggerSiblings(cyRef.current, 60, 8); // gap / 容差可调
                cyRef.current.fit();
              });

              // 首次渲染也执行一次（防止初始就共线）
              staggerSiblings(cyRef.current, 60, 8);
