...
const BfsDemo: React.FC = () => {
  ...
  const stepOnce = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      // 高亮“cur -> 入队邻居”的边（无向时任一方向都可）
      const newEdges: string[] = [];
      neighbors.forEach((n) => {
        const edge = elements.find(
          (el) =>
            (el.data?.source === cur && el.data?.target === n) ||
            (!isDirected &&
              el.data?.source === n &&
              el.data?.target === cur)
        );
    ...
    return true;
  }, [queue, visited, adjacency, elements, isDirected]);
  ...
  useEffect(() => {
    ...
    setHighlightedEdges([]);
    setNarration(
      `Start node set to ${startNode} (${isDirected ? "directed" : "undirected"}). Click Step/Play to begin.`
    );

    if (cyRef.current) {
      const layout = cyRef.current.layout({
        name: "breadthfirst",
        directed: isDirected, // 关键：根据模式决定是否按方向分层
        roots: `#${startNode}`,
        padding: 20,
        spacingFactor: 1.3,
        animate: true,
  ...
  const handleStart = () => {
    ...
    setCurrent(null);
    setHighlightedEdges([]);
    setNarration(`Ready. Starting from ${startNode} (${isDirected ? "directed" : "undirected"}).`);
    cyRef.current?.fit();
  };
  ...
  const handleReset = () => {
    ...
    setCurrent(null);
    setHighlightedEdges([]);
    setNarration(`Reset. Start from ${startNode} (${isDirected ? "directed" : "undirected"}).`);
    cyRef.current?.fit();
  };
  ...
  const stylesheet = [
    ...
    },
    {
      ...
      style: {
        width: 2,
        // 动态标签：有向显示 "A→B"，无向显示 "A—B"（可按需移除）
        label: (ele: any) =>
          isDirected
            ? `${ele.data("source")}→${ele.data("target")}`
            : `${ele.data("source")}—${ele.data("target")}`,
        "font-size": 10,
        ...
        "curve-style": "bezier", // 保持你当前的样式；若想直线可改为 "straight"
      },
    },
  ];
